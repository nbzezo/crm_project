...
/**
 * Doc nguoi thuc hien va ly do cua mot thay doi (v24, viet lai o v37).
 *
 * Truoc v37, actor doc tu query string (`?actor_contact_id=`) va lui ve co
 * singleton `contacts.is_me`. Ca hai deu het dung duoc khi he thong co nhieu
 * nguoi: mot client tu khai minh la ai thi nhat ky "ai doi cai gi" mat sach gia
 * tri, con mot co "toi" duy nhat thi khong con tra loi duoc cau hoi. Nay actor
 * luon lay tu phien dang nhap — KHONG BAO GIO tu dau vao cua client.
 *
 * `reason` thi van doc tu query string: do la mot y kien cua nguoi dung (ly do
 * ghi de cong giai doan, ghi chu Change Request), khong phai mot khang dinh ve
 * danh tinh, nen khong co gi de gia mao.
 *
 * Tra ve null khi tai khoan chua gan voi so danh ba: khuyet nguoi thuc hien van
 * tot hon la tu gan bua cho mot nguoi bat ky.
 */
export function auditFromRequest(req: Request): RecordOptions {
  const reason = req.query.reason;
  return {
    note: typeof reason === 'string' ? reason.trim() || null : null,
