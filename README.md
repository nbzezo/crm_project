...
- **AI Copilot đa nhà cung cấp** — cấu hình Gemini, Claude, DeepSeek và 9Router trong *Cài đặt*; API tự đọc danh sách model/capability, chọn model nhanh–cân bằng–suy luận và fallback khi provider lỗi. Trợ lý giữ ngữ cảnh hội thoại gần nhất, nhìn được ưu tiên hôm nay và Ghi chú nhanh; có AI Brief ở Dashboard/khách hàng/cơ hội, chuẩn hóa ghi chú tương tác, hỏi đáp CRM + tài liệu, hành động luôn cần duyệt, quota/token/chi phí, RAG tài liệu và automation cảnh báo chủ động.
...
Mở **Cài đặt → Trợ lý AI đa nhà cung cấp**, nhập API key cho Gemini, Claude, DeepSeek hoặc 9Router rồi bấm
